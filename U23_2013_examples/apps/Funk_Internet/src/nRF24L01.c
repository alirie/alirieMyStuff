...
void SPI2_Init(void)
{
  ...
  SPI_InitStructure.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_4;//²¨ÌØÂÊÔ¤·ÖÆµÖµÎª 256
  SPI_InitStructure.SPI_FirstBit = SPI_FirstBit_MSB;//Êý¾Ý´«Êä´Ó MSB Î»¿ªÊ¼
  SPI_InitStructure.SPI_CRCPolynomial = 7;//¶¨ÒåÁËÓÃÓÚ CRCÖµ¼ÆËãµÄ¶àÏîÊ½ 7
...
unsigned char SPI2_readReg(unsigned char reg)
{
   ...
   CSN_L();                    // CSNÖÃµÍ£¬¿ªÊ¼´«ÊäÊý¾Ý
   SPI2_readWrite(reg);                // Ñ¡Ôñ¼Ä´æÆ÷
   reg_val = SPI2_readWrite(0);        // È»ºó´Ó¸Ã¼Ä´æÆ÷¶ÁÊý¾Ý
   CSN_H();                    // CSNÀ­¸ß£¬½áÊøÊý¾Ý´«Êä
  ...
  while(1)
{
        ...
        RX_Mode();                            // ÉèÖÃÎª½ÓÊÕÄ£Ê½
        //TX_Mode(vEncoder);                    // °ÑnRF24L01ÉèÖÃÎª·¢ËÍÄ£Ê½²¢·¢ËÍÊý¾Ý
